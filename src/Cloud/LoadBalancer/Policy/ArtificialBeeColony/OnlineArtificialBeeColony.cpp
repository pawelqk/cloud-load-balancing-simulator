...
NodeToTaskMapping OnlineArtificialBeeColony::buildNodeToTaskMapping(const TaskPtrVec &tasks)
{
    ...
    for (auto iteration = 0; i < maxIterations; ++iteration)
    {
        // employed bees
        for (auto i = 0u; i < parameters.populationSize; ++i)
        {
            auto &currentSolution = solutions[i];
            const auto nextSolution = getNewSolutionFromNeighbourhood(solutions[i]);
        ...
        for (auto i = 0u; i < parameters.populationSize; ++i)
        {
            if (solution[i].counter > parameters.maxIterationsWithoutChange)
            {
    ...
    // TODO: validation
    return adjustSolutionWithExistingTasks(bestSolution.solution);
}
...
BestSolution OnlineArtificialBeeColony::generateRandomSolutions(const TaskPtrVec &tasks)
{
    ...
    for (auto i = 1u; i < parameters.maxIterationsWithoutChange; ++i)
    {
        solutions[i].solution = createRandomSolution(tasks);
        const auto currentValue = mappingAssessor->assess(solutions[i].solution);
        if (currentValue < bestValue)
...
NodeToTaskMapping OnlineArtificialBeeColony::createRandomSolution(const TaskPtrVec &tasks)
{
...
NodeToTaskMapping OnlineArtificialBeeColony::getNewSolutionFromNeighbourhood(const NodeToTaskMapping &solution)
{
...
std::vector<NodeId> OnlineArtificialBeeColony::extractFreeNodeIds()
{
...
std::vector<NodeId> OnlineArtificialBeeColony::findNotEmptyNodeIds(const NodeToTaskMapping &solution)
{
...
std::vector<NodeId> OnlineArtificialBeeColony::findFeasibleNodeIds(const NodeToTaskMapping &solution,
                                                                   const TaskPtr &task)
{
